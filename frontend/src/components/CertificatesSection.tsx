...
const CertificatesSection = () => {
  ...
  useEffect(() => {
    fetch("/api/certificates")
      ...
      .catch(() => {});
    fetch("/api/section-meta/certificates")
      .then((r) => r.json())
    ...
    <section id="certificates" ref={ref} className="py-24 md:py-32 relative overflow-hidden bg-secondary/10">
      ...
      <div className="container px-6 relative z-10">
        <div className="max-w-3xl mx-auto text-center mb-16 animate-in-view">
          <p className="text-sm tracking-[0.3em] uppercase text-primary mb-3 font-semibold">{sectionMeta?.subtitle || t("certificates.label")}</p>
          <h2 className="font-heading text-4xl md:text-6xl font-bold text-foreground mb-6">
            <span className="text-gradient">{sectionMeta?.title || t("certificates.title")}</span>
          </h2>
          ...
          <div className="overflow-hidden cursor-grab active:cursor-grabbing px-4" ref={emblaRef}>
            <div className="flex -ml-6">
              {certificates.map((cert, index) => (
                <div 
                  key={cert.id} 
                  className="flex-[0_0_85%] sm:flex-[0_0_45%] lg:flex-[0_0_32%] xl:flex-[0_0_23%] min-w-0 pl-6"
                >
                  <div 
                  ...
                  >
                    <div className="aspect-[3/3.5] w-full overflow-hidden bg-muted">
                      <img
                        src={cert.image_url}
