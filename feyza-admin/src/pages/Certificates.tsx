...
import { Award, Plus, Pencil, Trash2, Save, X, Loader2 } from "lucide-react";
  ...
  const handleAdd = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch("/api/certificates", {
        ...
        body: JSON.stringify({ image_url: b64, title: "", sort_order: 0 }),
      });
      if (!res.ok) { const err = await res.json().catch(() => ({})); toast.error(err.error || "Xatolik"); return; }
      toast.success("Qo'shildi");
      fetch("/api/certificates").then((r) => r.json()).then(setItems);
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-body font-bold">Sertifikatlar</h1>
          <p className="text-base text-foreground/70 mt-1">Sertifikatlarni boshqarish</p>
        </div>
        <input type="file" accept="image/*" ref={fileRef} onChange={handleAdd} className="hidden" />
        <button onClick={() => fileRef.current?.click()} disabled={uploading} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-pink-400 to-rose-400 text-white font-medium hover:from-pink-500 hover:to-rose-500 transition-all shadow-lg shadow-pink-200/50">
            ...
            {editing?.id === item.id ? (
              ...
              <>
                <div className="aspect-[4/3] bg-muted">
                  <img src={item.image_url} alt={item.title} className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = ""; }} />
                </div>
                <div className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Award className="w-4 h-4 text-primary" />
                    <p className="text-sm font-medium">{item.title || "Sertifikat"}</p>
                  </div>
